// Package client реализует распределённую раздачу заданий серверам и сбор
// результата с учётом кворума.
//
// Высокоуровневый алгоритм:
//  1. вход режется на N непрерывных чанков (по числу серверов);
//  2. каждый чанк параллельно отправляется на свой сервер через HTTP;
//  3. ответы собираются через канал; как только успешных ответов
//     накопилось >= quorum, итог считается достоверным;
//  4. результат собирается в исходном порядке (по ChunkID), даже если
//     ответы приходили вне порядка.
//
// Если кворум не достигнут, клиент всё равно печатает то, что удалось
// получить, и предупреждает stderr — это поведение удобно для отладки и
// соответствует требованиям задания "при достижении кворума".
package client
